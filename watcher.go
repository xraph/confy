...
type Watcher struct {
	...
	changeHandlers map[string]func(string, map[string]any)
...
func NewWatcher(config WatcherConfig) *Watcher {
	...
	return &Watcher{
		...
		changeHandlers: make(map[string]func(string, map[string]any)),
...
func (w *Watcher) WatchSource(ctx context.Context, source ConfigSource, callback WatchCallback) error {
	...
	if w.stopped {
		return ErrConfigError("watcher is stopped", nil)
	}
	...
	if w.metrics != nil {
		w.metrics.Counter("config.watcher.sources_started").Inc()
		w.metrics.Gauge("config.watcher.active_sources").Set(float64(len(w.sources)))
	}
...
func (w *Watcher) StopWatching(sourceName string) error {
	...
	if w.metrics != nil {
		w.metrics.Counter("config.watcher.sources_stopped").Inc()
		w.metrics.Gauge("config.watcher.active_sources").Set(float64(len(w.sources)))
	}
...
func (w *Watcher) StopAll() error {
	...
	w.changeHandlers = make(map[string]func(string, map[string]any))
	...
	if w.metrics != nil {
		w.metrics.Counter("config.watcher.stopped_all").Inc()
		w.metrics.Gauge("config.watcher.active_sources").Set(0)
	}
...
func (w *Watcher) GetSourceStats(sourceName string) (*WatchStats, error) {
	...
	watchCtx, exists := w.sources[sourceName]
	if !exists {
		return nil, ErrConfigError(fmt.Sprintf("source %s is not being watched", sourceName), nil)
	}
...
func (w *Watcher) checkForChanges(ctx context.Context, source ConfigSource, lastData *map[string]any, lastChecksum *string, callback WatchCallback) {
	sourceName := source.Name()
	...
	if *lastChecksum != "" && currentChecksum != *lastChecksum {
		...
		if w.metrics != nil {
			w.metrics.Counter("config.watcher.changes_detected").Inc()
		}
	}
...
func (w *Watcher) handleChange(sourceName string, oldData, newData map[string]any, callback WatchCallback) {
	if w.logger != nil {
...
func (w *Watcher) handleWatchError(sourceName string, err error) {
	...
	if w.metrics != nil {
		w.metrics.Counter("config.watcher.errors").Inc()
	}
...
func (w *Watcher) calculateChecksum(data map[string]any) string {
...
func (dw *DebounceWatcher) WatchSource(ctx context.Context, source ConfigSource, callback WatchCallback) error {
	...
	debouncedCallback := func(sourceName string, data map[string]any) {
...
func (dw *DebounceWatcher) debounceCallback(sourceName string, data map[string]any, callback WatchCallback) {
	dw.mu.Lock()
	defer dw.mu.Unlock()
