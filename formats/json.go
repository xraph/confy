...
func (p *JSONProcessor) Parse(data []byte) (map[string]any, error) {
	...
	if p.options.AllowComments || p.options.AllowTrailingCommas {
		...
		if err != nil {
			return nil, internal.ErrConfigError("failed to preprocess JSON5", err)
		}
	...
	if err := decoder.Decode(&result); err != nil {
		return nil, internal.ErrConfigError("failed to parse JSON", err)
	}
...
func (p *JSONProcessor) validateJSONStructure(data map[string]any) error {
	...
	if _, err := json.Marshal(data); err != nil {
		return internal.ErrValidationError("json_structure", err)
	}
