...
func (tp *TOMLProcessor) Parse(data []byte) (map[string]any, error) {
	...
	if _, err := decoder.Decode(&result); err != nil {
		return nil, internal.ErrConfigError(fmt.Sprintf("failed to parse TOML: %v", err), err)
	}

	// Convert the result to ensure proper types
	converted, err := tp.convertValue(result)
	if err != nil {
		return nil, internal.ErrConfigError(fmt.Sprintf("failed to convert TOML values: %v", err), err)
	}

	// In strict mode, perform additional validation after parsing
	if tp.strictMode {
		if err := tp.validateStrictMode(result); err != nil {
			return nil, internal.ErrConfigError(fmt.Sprintf("strict mode validation failed: %v", err), err)
		}
	}
	...
	return nil, internal.ErrConfigError("TOML root must be an object", nil)
}
...
func (tp *TOMLProcessor) validateValue(path string, value any) error {
	...
	default:
		...
		if tp.strictMode && !tp.allowUnknown {
			return internal.ErrValidationError(path, fmt.Errorf("unsupported value type: %T", value))
		}
...
func (tp *TOMLProcessor) ParseToStruct(data []byte, target any) error {
	...
	if _, err := decoder.Decode(target); err != nil {
		return internal.ErrConfigError(fmt.Sprintf("failed to decode TOML into struct: %v", err), err)
	}
...
func (tp *TOMLProcessor) FormatData(data map[string]any) ([]byte, error) {
	...
	if err != nil {
		return nil, internal.ErrConfigError(fmt.Sprintf("failed to prepare data for TOML: %v", err), err)
	}
	...
	if err := encoder.Encode(converted); err != nil {
		return nil, internal.ErrConfigError(fmt.Sprintf("failed to encode TOML: %v", err), err)
	}
...
func (tp *TOMLProcessor) ValidateStructTags(target any) error {
	...
	if targetType.Kind() != reflect.Struct {
		return internal.ErrConfigError("target must be a struct", nil)
	}
...
func (tp *TOMLProcessor) validateTOMLTag(tag, fieldPath string) error {
	...
	if name != "" && !tp.isValidTOMLFieldName(name) {
		return internal.ErrValidationError(fieldPath, fmt.Errorf("invalid TOML field name: %s", name))
	}
	...
	for i := 1; i < len(parts); i++ {
		...
		if !tp.isValidTOMLOption(option) {
			return internal.ErrValidationError(fieldPath, fmt.Errorf("invalid TOML tag option: %s", option))
		}
	}
	...
	case map[string]any:
		for key, val := range v {
			...
			if !tv.processor.isValidTOMLFieldName(key) {
				return internal.ErrValidationError(keyPath, fmt.Errorf("invalid TOML table key: %s", key))
			}
