...
func (p *YAMLProcessor) Parse(data []byte) (map[string]any, error) {
	...
	if err := decoder.Decode(&result); err != nil {
		return nil, internal.ErrConfigError("failed to parse YAML", err)
	}
...
func (p *YAMLProcessor) Validate(data map[string]any) error {
	if data == nil {
		return internal.ErrValidationError("data", errors.New("configuration data is nil"))
	}
...
func (p *YAMLProcessor) processYAMLData(data map[string]any) (map[string]any, error) {
	...
	for key, value := range data {
		...
		if err != nil {
			return nil, internal.ErrConfigError(fmt.Sprintf("failed to process key '%s'", key), err)
		}
...
func (p *YAMLProcessor) checkCircularReferences(data any, visited map[any]bool) error {
	...
	if visited[dataPtr.Interface()] {
		return internal.ErrValidationError("circular_reference", errors.New("circular reference detected"))
	}
...
func (p *YAMLProcessor) validateNestedStructure(data map[string]any, path string) error {
	for key, value := range data {
		...
		if err := p.validateKeyFormat(key); err != nil {
			return internal.ErrValidationError(currentPath, err)
		}
...
func (p *YAMLProcessor) checkRequiredFields(data map[string]any) error {
	for _, required := range p.options.RequiredFields {
		if !p.hasNestedKey(data, required) {
			return internal.ErrValidationError("required_field", fmt.Errorf("required field missing: %s", required))
		}
	}
...
func (p *YAMLProcessor) checkDeprecatedFields(data map[string]any) error {
	...
	if len(warnings) > 0 && p.options.Strict {
		return internal.ErrValidationError("deprecated_fields", errors.New(strings.Join(warnings, "; ")))
	}
...
func (p *YAMLProcessor) ParseMultiDocument(data []byte) ([]map[string]any, error) {
	...
	for {
		...
		if err != nil {
			...
			return nil, internal.ErrConfigError("failed to parse YAML document", err)
		}
...
func (p *YAMLProcessor) ValidateWithSchema(data map[string]any, schema any) error {
	...
	if err != nil {
		return internal.ErrValidationError("json_conversion", err)
	}
