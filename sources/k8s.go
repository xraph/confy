...
func NewK8sSource(options K8sSourceOptions) (configcore.ConfigSource, error) {
	...
	if err != nil {
		return nil, configcore.ErrConfigError(fmt.Sprintf("failed to create Kubernetes config: %v", err), err)
	}

	// Create Kubernetes clientset
	clientset, err := kubernetes.NewForConfig(config)
	if err != nil {
		return nil, configcore.ErrConfigError(fmt.Sprintf("failed to create Kubernetes client: %v", err), err)
	}
	...
	if err := source.testConnection(context.Background()); err != nil {
		return nil, configcore.ErrConfigError(fmt.Sprintf("failed to connect to Kubernetes: %v", err), err)
	}
...
func (ks *K8sSource) Watch(ctx context.Context, callback func(map[string]any)) error {
	...
	if ks.watching {
		return configcore.ErrConfigError("already watching Kubernetes resources", nil)
	}

	if !ks.IsWatchable() {
		return configcore.ErrConfigError("Kubernetes watching is not enabled", nil)
	}
...
func (ks *K8sSource) GetSecret(ctx context.Context, key string) (string, error) {
	...
	if len(parts) != 2 {
		return "", configcore.ErrConfigError(fmt.Sprintf("invalid secret key format: %s (expected secret-name/key)", key), nil)
	}
	...
	if err != nil {
		return "", configcore.ErrConfigError(fmt.Sprintf("failed to get secret from Kubernetes: %v", err), err)
	}

	if secret.Data == nil {
		return "", configcore.ErrConfigError("secret data is nil: "+secretName, nil)
	}

	data, exists := secret.Data[secretKey]
	if !exists {
		return "", configcore.ErrConfigError(fmt.Sprintf("secret key not found: %s in %s", secretKey, secretName), nil)
	}
...
func (ks *K8sSource) loadConfigMapsWithSelectors(ctx context.Context, config map[string]any) error {
	...
	if err != nil {
		return configcore.ErrConfigError(fmt.Sprintf("failed to list ConfigMaps: %v", err), err)
	}
...
func NewK8sSourceFactory(logger logger.Logger, errorHandler errors.ErrorHandler) *K8sSourceFactory {
