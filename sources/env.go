...
func (es *EnvSource) Watch(ctx context.Context, callback func(map[string]any)) error {
	...
	if es.watching {
		return configcore.ErrConfigError("already watching environment variables", nil)
	}

	if !es.IsWatchable() {
		return configcore.ErrConfigError("environment variable watching is not enabled", nil)
	}
...
func (es *EnvSource) GetSecret(ctx context.Context, key string) (string, error) {
	...
	for _, secretVar := range es.options.SecretVars {
		if secretVar == key {
			value := os.Getenv(key)
			if value == "" {
				return "", configcore.ErrConfigError("secret environment variable not found: "+key, nil)
			}
	...
	return "", configcore.ErrConfigError(fmt.Sprintf("key %s is not configured as a secret variable", key), nil)
}
...
func NewEnvSourceFactory(logger logger.Logger, errorHandler errors.ErrorHandler) *EnvSourceFactory {
