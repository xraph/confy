...
func NewFileSource(path string, options FileSourceOptions) (configcore.ConfigSource, error) {
	if path == "" {
		return nil, configcore.ErrConfigError("file path cannot be empty", nil)
	}
	...
	if err != nil {
		return nil, configcore.ErrConfigError("unsupported format: "+format, err)
	}
...
func (fs *FileSource) Load(ctx context.Context) (map[string]any, error) {
	...
	if err != nil {
		...
		return nil, configcore.ErrConfigError("failed to stat file "+path, err)
	}
	...
	if err != nil {
		return nil, configcore.ErrConfigError("failed to parse file "+path, err)
	}
	...
	if fs.options.ExpandSecrets {
		...
		if err != nil {
			return nil, configcore.ErrConfigError("failed to expand secrets in "+path, err)
		}
	}
	...
	if validator, ok := fs.processor.(interface {
		...
		if err := validator.Validate(data); err != nil {
			return nil, configcore.ErrConfigError("validation failed for file "+path, err)
		}
	}
...
func (fs *FileSource) Watch(ctx context.Context, callback func(map[string]any)) error {
	...
	if fs.watching {
		return configcore.ErrConfigError("already watching file", nil)
	}

	if !fs.IsWatchable() {
		return configcore.ErrConfigError("file watching is not enabled", nil)
	}

	watcher, err := fsnotify.NewWatcher()
	if err != nil {
		return configcore.ErrConfigError("failed to create file watcher", err)
	}
...
func (fs *FileSource) GetSecret(ctx context.Context, key string) (string, error) {
	...
	value := os.Getenv(key)
	if value == "" {
		return "", configcore.ErrConfigError("secret not found: "+key, nil)
	}
...
func NewFileSourceFactory(logger logger.Logger, errorHandler errors.ErrorHandler) *FileSourceFactory {
