...
func NewConsulSource(prefix string, options ConsulSourceOptions) (configcore.ConfigSource, error) {
	...
	if err != nil {
		return nil, configcore.ErrConfigError(fmt.Sprintf("failed to create Consul client: %v", err), err)
	}
	...
	if err := source.testConnection(context.Background()); err != nil {
		return nil, configcore.ErrConfigError(fmt.Sprintf("failed to connect to Consul: %v", err), err)
	}
...
func (cs *ConsulSource) Load(ctx context.Context) (map[string]any, error) {
	...
	if err != nil {
		return nil, configcore.ErrConfigError(fmt.Sprintf("failed to query Consul KV: %v", err), err)
	}
...
func (cs *ConsulSource) Watch(ctx context.Context, callback func(map[string]any)) error {
	...
	if cs.watching {
		return configcore.ErrConfigError("already watching Consul KV", nil)
	}

	if !cs.IsWatchable() {
		return configcore.ErrConfigError("Consul watching is not enabled", nil)
	}
...
func (cs *ConsulSource) GetSecret(ctx context.Context, key string) (string, error) {
	...
	if err != nil {
		return "", configcore.ErrConfigError(fmt.Sprintf("failed to get secret from Consul: %v", err), err)
	}

	if pair == nil {
		return "", configcore.ErrConfigError("secret not found in Consul: "+key, nil)
	}
...
func NewConsulSourceFactory(logger logger.Logger, errorHandler errors.ErrorHandler) *ConsulSourceFactory {
